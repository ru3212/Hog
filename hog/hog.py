...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while not reached_goal:
        ...
        if who == 0:
            ...
            commentary0 = commentary0(score0, score1)
            if score0 >= goal:
                reached_goal = True
            while extra_turn(score0, score1) and not reached_goal:
                score0 += take_turn((strategy0(score0, score1)), score1, dice)
                commentary0 = commentary0(score0, score1)
                if score0 >= goal:
                    reached_goal = True
        ...
        if who == 1:
            ...
            if score1 >= goal:
                reached_goal = True
            while extra_turn(score1, score0) and not reached_goal:
                score1 += take_turn((strategy1(score1, score0)), score0, dice)
                commentary0 = commentary0(score0, score1)
                if score1 >= goal:
                    reached_goal = True
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        latestscore = last_score
        if who == 1:
            player = score1
        else:
            player = score0
        if (new_high < (player - last_score)):
            new_high = (player - last_score)
            print(new_high, 'point(s)! The most yet for Player', who)
        lastestscore = player
        return announce_highest(who, lastestscore, new_high)
    return say
    "*** YOUR CODE HERE ***"
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    trials = trials_count
    dices = dice
    avg = make_averaged(roll_dice, trials)
    while k <= 10:
        new_avg = avg(k, dices)
        if (new_avg / trials_count) > (high_avg / trials_count):
            high_avg = new_avg
            max_dice = k
        k += 1
    return max_dice
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    zero_dice_points = free_bacon(opponent_score)
    if zero_dice_points >= cutoff:
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    rolling_zero = free_bacon(opponent_score)
    if extra_turn((score + rolling_zero), opponent_score):
        return 0
    elif rolling_zero >= cutoff:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
